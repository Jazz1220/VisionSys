...
void morphOps(Mat &thresh)
{
    ...
    gpu::GpuMat erodeElement = getStructuringElement( MORPH_RECT, Size(5,5));
 
    //Element chosen here is a 1px by 1px rectangle for dilation
    gpu::GpuMat dilateElement = getStructuringElement( MORPH_RECT, Size(1,1));
 
    //erode(thresh, thresh, erodeElement, Point(-1,-1), Iterations);
    gpu::erode(thresh, thresh, erodeElement, Point(-1,-1), 1);
 
    gpu::dilate(thresh, thresh, dilateElement, Point(-1,-1), Iterations);
}
...
int main() {
	...
	gpu::GpuMat gpuThreshMat(640, 480, CV_8U, Scalar(0));
	...
	while (die == false) {
		...
		gpuThreshMat = threshMat;
		morphOps(gpuThreshMat);
		threshMat = gpuThreshMat;
