...
main()
{
    ...
    Mat
    bgrMatPost(Size(640, 480), CV_8UC3, Scalar(0));
    Mat
	...
	inRange(hsvMat, Scalar(100, 65, 21), Scalar(140, 255, 255),
		threshMat);
	...
	adaptiveThreshold(threshMat, bgrThresh, 255,
			  CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, 3,
			  1);

	detector.detect(threshMat, Keypoints);
	drawContours(postProc, contours, -1, Scalar(255, 255, 255), 1,
		     8, hierarchy, INT_MAX, Point(0, 0));
	drawKeypoints(bgrMat, Keypoints, postProc, Scalar(0, 0, 255));
