...
//Thanks to Frederic Philips for the code; without you, this thresholding would be a lot harder!
...
int main() {
	...
	Mat invThreshMat(Size(640, 480), CV_8U, Scalar(0));
	Mat invBGRThresh(Size(640, 480), CV_8U, Scalar(0));
	Mat maskedThresh(Size(640, 480), CV_8U, Scalar(0));
	Mat blackMat(Size(640, 480), CV_8U, Scalar(0));
	Mat maskedDepth(Size(640, 480), CV_8UC1);
	...
	while (die == false) {
		...
		inRange(hsvMat, Scalar(100, 65, 21), Scalar(140, 255, 255), threshMat);
		...
		//adaptiveThreshold( outFrame, bgrThresh, 255, CV_ADAPTIVE_THRESH_GAUSSIAN_C, CV_THRESH_BINARY, 3, 1);		
		...
		drawKeypoints(bgrMat, Keypoints, postProc, Scalar(0, 0, 255)/*, DrawMatchesFlags::DRAW_RICH_KEYPOINTS*/);
		drawContours( postProc, contours, -1, Scalar( 255, 255, 255), 1, 8, hierarchy, INT_MAX, Point(0, 0));
