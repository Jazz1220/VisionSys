...
main()
{
    ...
    createTrackbar("Minimum Threshold:", "Post", &lowThreshold, 100);
    createTrackbar("Hue:", "Post", &hue, 180);
    createTrackbar("Saturation:", "Post", &saturation, 255, 90);
    createTrackbar("Value:", "Post", &value, 255, 100);
    // , 
									// 
    // 
    // 
	...
	inRange(hsvMat, Scalar(hue - 15, saturation - 30, value - 30), Scalar(hue + 15, 255, 255),
		threshMat);
	...
	//drawKeypoints(bgrMat, Keypoints, postProc, Scalar(0, 0, 255)	/* ,
	//								 * DrawMatchesFlags::DRAW_RICH_KEYPOINTS */ );
	drawContours(postProc, contours, -1, Scalar(0, 0, 255), 2, 8,
	...
	char            k = cvWaitKey(1);
	/*
	 * if (k == 119) { setMotorPosition(MotorPos + 10); } 
	 */
	if (k == 27) {
	    cvDestroyWindow("rgb");
