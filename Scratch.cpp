...
int main() {
	...
	Mat mask(Size(640, 480), CV_8UC1, Scalar(0));
	...
	while (die == false) {
		...
		//stream.read(inFrame);
		//stream.grab();
		//stream.retrieve(inFrame);
		cv::cvtColor(rgbMat, outFrame, CV_RGB2GRAY);
		detector.detect(outFrame, Keypoints);
		//FAST(inFrame, Keypoints, 30);
		
		if (Keypoints.empty())
		{
		exit(11);	
		}
		drawKeypoints(rgbMat, Keypoints, postProc, Scalar(0, 0, 255)/*, DrawMatchesFlags::DRAW_RICH_KEYPOINTS*/);
